<tr id="{{ $id }}">
    <td class="with-btn" >
        <select class="form-control selectpicker" name="barang_masuk[{{ $id }}][barang_id]" data-live-search="true" data-style="btn-aqua" data-size="3" data-width="100%">
            ...
            <option value="{{ $row->barang_id }}" {{ $data && $data['barang_id'] == $row->barang_id? 'selected': '' }}>{{ $row->barang_nama }}</option>
            @endforeach
        </select>
    </td>
    <td class="with-btn width-100">
        <input class="form-control" type="number" name="barang_masuk[{{ $id }}][barang_masuk_qty]" value="{{ $data? $data['barang_masuk_qty']: 0 }}" autocomplete="off" min="1" required/>
    </td>
    <td class="with-btn width-200">
        <input class="form-control text-right currency" id="harga{{ $id }}" type="text" name="barang_masuk[{{ $id }}][barang_masuk_harga_barang]" value="{{ $data? $data['barang_masuk_harga_barang']: 0 }}" autocomplete="off" required/>
    </td>
    <td class="with-btn width-200">
