<tr id="{{ $id }}">
    <td class="with-btn">
        <select class="form-control selectpicker barang_id" title="Nama Barang" id="barang{{ $id }}"
            onchange="satuan({{ $id }})" name="barang[{{ $id }}][barang_id]" data-live-search="true"
    ...
    <td class="with-btn width-20">
        <select class="form-control selectpicker satuan" name="barang[{{ $id }}][satuan_nama]"
    ...
    <td class="with-btn width-150">
        <input type="text" class="form-control text-right" onchange="total_harga_barang({{ $id }})"
            ...
            name="barang[{{ $id }}][satuan_harga]" value="{{ $data ? $data['satuan_harga'] : 0 }}"
    ...
    <td class="with-btn width-100">
        <input class="form-control qty" onchange="total_harga_barang({{ $id }})"
            ...
            name="barang[{{ $id }}][penjualan_detail_qty]"
            value="{{ $data ? $data['penjualan_detail_qty'] : 0 }}" autocomplete="off" min="1" required />
    </td>
    <td class="with-btn width-100">
    ...
    <td class="with-btn width-150">
        <input type="text" class="form-control text-right total-harga-barang"
            name="barang[{{ $id }}][penjualan_detail_total]" id="total{{ $id }}"
